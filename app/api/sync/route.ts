...
// In-memory job store (persists as long as Lambda is warm)
interface SyncJob {
  id: string
  status: "fetching" | "classifying" | "done" | "error"
  message: string
...
setInterval(() => {
  ...
  for (const [id, job] of jobs.entries()) {
    if (now - job.lastUpdated > 10 * 60 * 1000) {
      jobs.delete(id)
    }
  }
}, 5 * 60 * 1000)
...
export async function POST(request: Request) {
  ...
  const job: SyncJob = {
    id: jobId,
    status: "fetching",
    message: "Searching Gmail for job emails...",
    applications: [],
    totalThreads: 0,
    fetchedThreads: 0,
    processedBatches: 0,
  ...
  // We don't 'await' this so we can return the jobId immediately
  ;(async () => {
    try {
      ...
      for (const q of queries) {
        if (Date.now() - startTime > 25000) break // Amplify timeout guard
        ...
        } while (pageToken && (Date.now() - startTime < 25000))
      }

      if (allThreadIds.size === 0) {
        job.status = "done"
        job.message = "No job-related emails found."
        job.lastUpdated = Date.now()
        return
      }

      job.status = "fetching"
      job.message = `Found ${allThreadIds.size} potential threads. Fetching details...`
      job.totalThreads = allThreadIds.size
      job.lastUpdated = Date.now()
      ...
      for (const threadId of Array.from(allThreadIds)) {
        ...
        try {
          ...
          job.fetchedThreads++
          if (job.fetchedThreads % 10 === 0) {
            job.message = `Fetched ${job.fetchedThreads}/${allThreadIds.size} threads...`
            job.lastUpdated = Date.now()
          }
        } catch (err) {
      ...
      if (filteredThreads.length === 0) {
        job.status = "done"
        job.message = "No job-related threads passed filtering."
        job.lastUpdated = Date.now()
        return
      }
      ...
      const BATCH_SIZE = 10 
      job.totalBatches = Math.ceil(filteredThreads.length / BATCH_SIZE)
      
      for (let i = 0; i < filteredThreads.length; i += BATCH_SIZE) {
        if (Date.now() - startTime > 25000) {
          job.message = "Processing limit reached. Showing partial results."
          break
        }
        const batch = filteredThreads.slice(i, i + BATCH_SIZE)
        job.processedBatches++
        job.message = `Classifying batch ${job.processedBatches} of ${job.totalBatches}...`
        job.lastUpdated = Date.now()
        ...
        job.applications.push(...batchApps)
        job.lastUpdated = Date.now()
      }

      job.status = "done"
      job.message = `Sync complete! Found ${job.applications.length} applications.`
      job.lastUpdated = Date.now()

    } catch (error: any) {
      console.error("Sync background error:", error)
      job.status = "error"
      job.error = error.message || "Sync failed"
      job.lastUpdated = Date.now()
    }
  })()
